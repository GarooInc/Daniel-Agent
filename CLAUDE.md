...
- **Debounce/cola con BullMQ** (`messaging/debounce-queue.ts`) agrupa mensajes rápidos del mismo usuario antes de invocar a `askDaniel` una sola vez. Bug conocido sin arreglar: el `jobId` solo usa `slackUserId`, no el canal — si el mismo usuario escribe en dos canales dentro de la ventana de debounce, la respuesta puede terminar en el canal equivocado (ver ESTADO-PROYECTO.md, punto 16).
