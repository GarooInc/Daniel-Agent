...
0. **(Próximo paso, único pendiente bloqueante) Reintentar en vivo el flujo completo de escalación — dos intentos fallidos hasta ahora (2026-07-30, tarde), dos bugs reales encontrados y arreglados, limpieza de estado ya hecha (2026-07-31), pero ningún fix reprobado todavía.**
   ...
   - **Limpieza completada (2026-07-31)**: los dos documentos contaminados fueron borrados. `chat_histories`/`ticket_drafts` para `slackUserId U0BJZDZRSQZ` se inspeccionaron antes de borrar y coincidían exactamente con lo esperado (la conversación de facturación del widget-chatbot mezclada con el ticket `3127078337` y un mensaje posterior sin resolver) — `deleteOne` confirmado en ambas colecciones (`deletedCount: 1`). El ticket `3127078337` en el board `5101177200` ya figuraba con `state: "deleted"` en Monday (borrado previamente), no hizo falta acción ahí. Nota técnica: el borrado se corrió desde una máquina Mac nueva conectándose directo con el `MONGODB_URI` (SRV) sin necesitar el workaround de DNS-over-HTTPS que hacía falta en la máquina Windows — parece un problema específico de esa red, no del connection string en sí.
   - **Nota para máquinas nuevas**: si vas a correr scripts contra Mongo/Monday desde un checkout recién clonado, correr `npm install` primero — en esta sesión (Mac) `node_modules` no tenía `mongodb` instalado pese a estar en `package.json`.
   - **Para la próxima prueba (único pendiente bloqueante que queda)**: repetir el mismo tipo de caso (datos de a uno por mensaje) y confirmar que: (a) el ticket creado refleja el problema real dicho en la conversación, (b) no repregunta datos ya dados, (c) no "olvida" ni "reinventa" una escalación, (d) aparece el aviso en `#escalacion`, (e) el perfil queda guardado. Si vuelve a fallar con datos ajenos a la conversación pese a los dos fixes, no asumir que alcanza con más limpieza de estado — puede haber una tercera vía (p. ej. el modelo de extracción infiriendo de más pese a la instrucción de no inventar) que requiera revisar `extract-ticket-fields.ts` directamente.
