...
- [x] **Extracción de email por regex + acumulación progresiva del ticket (2026-07-30)**: pruebas en vivo mostraron que `gpt-5-mini` no recuperaba de forma confiable datos sueltos dados varios turnos atrás (nombre, producto, resumen), aunque el historial completo estuviera en el contexto — ni ajustando el prompt dos veces seguidas. Dos fixes, en capas:
  1. `daniel.ts` extrae por regex cualquier email mencionado por el cliente en el hilo (no solo el último mensaje) y lo inyecta como dato ya conocido en el system prompt — determinístico, no depende de que el modelo "preste atención".
  2. Cambio más de fondo: `escalar_a_monday` (mismo archivo) ahora acepta argumentos parciales, los combina con un borrador persistido en la nueva colección `ticket_drafts` (`src/integrations/mongo/ticket-draft.ts`) y con el perfil conocido, y solo crea el ticket real en Monday cuando los 6 campos requeridos están completos — devolviéndole al modelo exactamente qué falta si no. Así la "memoria" del ticket en construcción vive en Mongo, no en la cabeza del modelo entre turnos. El prompt se simplificó para reflejar esto: llamar a la tool con lo que se tenga apenas se tenga, no esperar a juntarlo todo antes de llamarla.
  - **Aprendizaje para futuras sesiones**: si aparece otro caso de "Daniel no recuerda algo que ya le dijeron", la solución más confiable no es seguir puliendo el prompt — es sacar ese dato de la memoria del LLM y ponerlo en almacenamiento determinístico (regex, tool con estado en Mongo, etc.), como se hizo acá.
...
Los 3 pasos de prueba end-to-end (askDaniel en vivo, Slack + agente en vivo, escalación a Monday desde Slack) ya se confirmaron en vivo el 2026-07-29 — ver detalle arriba. `MONGODB_URI`/`MONGODB_DB_NAME=DanielSoporte` ya están cargadas en Coolify y Mongo está confirmado funcionando en producción (2026-07-30). El diseño nuevo de acumulación progresiva del ticket (`ticket_drafts`) todavía está en la primera prueba en vivo pendiente de confirmar. Queda:
