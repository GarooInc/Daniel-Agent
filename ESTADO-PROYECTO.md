...
- [~] **Retest en vivo en Slack (2026-07-31, sesión noche): ticket creado en Monday, pero el aviso a `#escalacion` sigue sin llegar.** Ana López (cliente de prueba) escaló un caso urgente por Slack; Daniel creó el ticket real en Monday (id `3130442122`) sin problema. La notificación a `#escalacion` (`notify-escalation.ts`) no llegó — mismo síntoma que la sesión anterior, ya sin resolver.
  ...
  - **Retest 2026-08-04: los 3 datos pendientes ya se confirmaron, y los tres descartan las hipótesis originales — el misterio sigue abierto.**
    1. **Bot Token Scopes confirmados completos**: `app_mentions:read`, `channels:history`, `channels:read`, `chat:write`, `groups:history`, `groups:read`, `im:history`, `im:write`, `mpim:history`. No falta `channels:read`/`groups:read` (necesarios para que `conversations.list` encuentre el canal) ni `chat:write`. No tiene `chat:write.public`, pero no debería hacer falta porque:
    2. **`@Daniel-Soporte` confirmado invitado al canal `escalacion`** — con esto, `chat:write.public` es innecesario (alcanza con ser miembro del canal).
    3. **Ticket de prueba nuevo generado y confirmado creado en Monday (id `3138576597`, 2026-08-04) — otra vez sin aviso en `#escalacion`.** Pero esta vez, al revisar el log de Coolify (con F5) inmediatamente después, **el log de la app no tenía absolutamente ninguna línea sobre el mensaje procesado ni el ticket creado** — solo las 2 líneas de arranque del proceso (`"Daniel está corriendo"` + `"Redis conectado"`), a pesar de que la conversación en Slack sí muestra a Daniel respondiendo con el ticket real. El header de Coolify mostraba el estado del recurso como `"Running (unknown)"` con un ícono ⚠️ junto al nombre — señal de que Coolify mismo no está seguro del estado, lo que hace sospechar que el panel de Logs puede estar mostrando el stream de un contenedor viejo/huérfano mientras otro contenedor (el que realmente sostiene la conexión Socket Mode con Slack) sigue corriendo sin que lo veamos — un escenario ya visto antes en este proyecto (el bug original de tickets duplicados por más de una conexión Socket Mode activa a la vez).
    - **Intentado sin éxito**: acceso SSH directo a la VPS (`82.29.180.111`, usuario `root`) desde el entorno de Claude Code para poder correr `docker ps`/`docker logs` sin depender del panel web — **conexión rechazada por timeout en los puertos 22, 443 y 8000**, pese a que el acceso general a internet desde ese entorno funciona bien (confirmado contra `api.github.com`). No se identificó la causa (¿firewall del lado del entorno de Claude Code, no de la VPS?) y no se seguirá insistiendo por este camino — seguimos dependiendo de capturas manuales del panel de Coolify.
    - **Próximo paso acordado, sin ejecutar todavía**: hacer **Restart** (no Redeploy) del recurso `daniel-agent` en Coolify para forzar que quede una única instancia limpia corriendo, y recién ahí repetir el test de escalación y volver a mirar el log fresco — así se descarta de una vez la hipótesis del contenedor duplicado/huérfano antes de seguir buscando en otro lado.
...
1. **Bug real y arreglado — asimetría entre el texto embebido de la consulta y el de la FAQ.** El modelo llamó a `buscar_faqs` con `query: "no me propone horarios para juntarme con un cliente"` y `producto: "Isabella"` — omitiendo "Isabella" del texto porque ya lo mandó separado como filtro. Pero el texto guardado de cada FAQ sí menciona el producto (la pregunta dice "...Isabella..."), así que esa asimetría le restaba score real a un match correcto (0.72 en vez de 0.77) sin que el match fuera peor semánticamente — quedó justo por debajo del umbral (0.75) y Daniel dijo "no encontré nada". Confirmado con el log de debug agregado esta sesión (`DEBUG buscar_faqs`, ver `search-faqs.ts`). Fix: si viene `producto`, se antepone al texto que se embebe (`"Isabella: no me propone..."` en vez de solo la consulta) — el match subió a 0.77. De paso, se bajó el umbral de 0.75 a 0.72 (con margen: una consulta irrelevante de prueba, "cual es el clima en Guatemala", no pasó de 0.68 incluso con el mismo prefijo de producto). Type-check limpio, 44/44 tests verdes. **Pendiente**: pushear y reprobar en vivo; una vez confirmado, sacar el logging `DEBUG buscar_faqs` (temporal, agregado para este diagnóstico).

2. **Bug de infra, una sola vez, autorresuelto — posible cold-start de Mongo tras el redeploy.** El primer mensaje enviado justo después de un redeploy falló con `MongoServerSelectionError: Server selection timed out after 30000 ms` — de los 3 hosts del replica set, uno (el que los otros dos reportan como primary) apareció como `"Unknown"`/no alcanzable. Esto tumbó todo `askDaniel()` (no solo la FAQ) y disparó la auto-escalación de emergencia con datos genéricos (ticket `#3141988009`). El segundo mensaje, sin ningún cambio de código de por medio, conectó sin problema. Consistente con un blip transitorio de conexión en el arranque en frío del contenedor (el pool de conexiones de MongoDB recién se establece con la primera query real), no con un problema persistente — pero **sin confirmar del todo**, queda como algo a vigilar si se repite en futuros redeploys.

3. **Bug de config de Slack, sin código de por medio — falta el scope `users:read`.** El fallback de auto-escalación intenta buscar el nombre real del cliente vía `client.users.info()` y falla con `missing_scope` (scopes actuales: `chat:write, app_mentions:read, im:write, im:history, channels:history, groups:history, mpim:history, channels:read, groups:read` — falta `users:read`). No es fatal (usa `"Usuario de Slack {id}"` de respaldo y el ticket se crea igual), pero degrada la calidad del dato en tickets de auto-escalación. **Fix pendiente, no bloqueante**: agregar `users:read` en OAuth & Permissions de la app y reinstalar.

**Pendiente**: pushear el código (todavía sin confirmar con Jorge), reprobar en vivo el fix del umbral, sacar el logging de debug una vez confirmado, agregar el scope `users:read`, y en algún momento futuro migrar `customers.json` a una colección Mongo plana (sin vectores).
...
0. **(Parece resuelto, sin confirmación 100%) Rotar los tokens de Slack parece haber eliminado la instancia fantasma** — ver "Retest en vivo (2026-08-05)" arriba. Un test de mensaje único por DM dio una sola respuesta, sin duplicados. Falta más confirmación en uso real (varios días sin síntomas) antes de darlo por cerrado del todo.

1. **(HECHO y reprobado en vivo, 2026-08-05) `clearHistory()` agregado a la rama `isNewSession` de `daniel.ts`** — ver "Hallazgo mayor" arriba para el detalle del fix y del cuidado de la carrera con `appendMessage`. Deployado (`57f75a1`), confirmado funcionando en el retest de esta misma sesión.

1b. **(HECHO y reprobado en vivo, 2026-08-05) `""` tratado como ausente en `mergeTicketFields` + prompt de extracción corregido** — ver "Retest en vivo (2026-08-05)" arriba. Bug encontrado en vivo (Daniel repreguntaba el nombre ya guardado en el perfil), confirmado con query directa a Mongo, deployado (`57f75a1`) y confirmado funcionando: creó el ticket `#3141811663` de una sin repreguntar. Nota menor: ese retest dio el nombre explícito en el mismo turno, así que no puso a prueba el caso exacto de "usar el nombre del perfil sin que el cliente lo repita" — pendiente de una prueba dedicada a eso si vuelve a aparecer el síntoma.

2. **(RESUELTO, confirmado 2 veces en vivo el 2026-08-05) El aviso a `#escalacion` llega completo y de forma consistente** — tickets `#3141838626` y `#3141811663`, ambos notificados sin problema. Causa real, con alta confianza: la instancia fantasma (punto 0) era la que fallaba en notificar, no un problema de scopes/invitación (ya descartados antes) ni del código de `notify-escalation.ts`.
