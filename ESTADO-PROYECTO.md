...
Type-check limpio, 37/37 tests verdes. **Pendiente**: reprobar en vivo — no hace falta limpiar el `ticket_draft` contaminado a mano en Mongo, el fix ya deployado va a ignorar el `""` y tomar el nombre del perfil correctamente la próxima vez que se recalcule el borrador.
...
1. **(HECHO 2026-08-05) `clearHistory()` agregado a la rama `isNewSession` de `daniel.ts`** — ver "Hallazgo mayor" arriba para el detalle del fix y del cuidado de la carrera con `appendMessage`. Type-check + 37/37 tests verdes. Falta reprobar en vivo.

1b. **(HECHO 2026-08-05) `""` tratado como ausente en `mergeTicketFields` + prompt de extracción corregido** — ver "Retest en vivo (2026-08-05)" arriba. Bug encontrado en vivo (Daniel repreguntaba el nombre ya guardado en el perfil) y confirmado con query directa a Mongo. Type-check + 37/37 tests verdes. Falta reprobar en vivo.

2. **(Parece resuelto) El aviso a `#escalacion` llegó completo en el retest del 2026-08-05** (ticket `#3141838626`) — ver "Retest en vivo" arriba. Sospecha: la causa real todo este tiempo era la instancia fantasma (punto 0), no scopes/invitación (ya descartados en sesiones anteriores) ni el código de `notify-escalation.ts`. Falta confirmar que se sostiene en más pruebas antes de cerrar del todo.
