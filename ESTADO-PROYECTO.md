...
7. **KB vectorizada en Mongo Atlas — HECHO para FAQs, confirmado end-to-end en Slack (2026-08-06)** — ver "Paso 5" arriba para el detalle completo (schema calcado de `Centralizado.documents`, embeddings vía OpenRouter, migración corrida en producción, búsqueda semántica probada con una consulta sin keywords compartidas, y reprobada a través del agente completo en Slack tras arreglar la asimetría del texto embebido — ver el retest del mismo día). De paso, este retest destapó y arregló 2 bugs más: el cold-start de Mongo en cada redeploy (punto 2 arriba) y el scope `users:read` faltante en Slack (punto 3 arriba). **Pendiente**: migrar `customers.json` a una colección Mongo plana (sin vectores) en algún momento futuro.
